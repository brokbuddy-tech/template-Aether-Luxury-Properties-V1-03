...
export function OffPlanCard({ project }: { project: OffPlanProject }) {
  ...
  const calculateStartingPrice = () => {
    if (project.paymentPlan && project.paymentPlan.length > 0) {
      ...
      if (firstMilestone.percentage > 0) {
        return firstMilestone.amount / (firstMilestone.percentage / 100);
      }
    }
    return 0;
  };
  ...
  return (
    <div className="relative group">
      <Card className="flex flex-col w-full max-w-[350px] mx-auto border border-gray-200 rounded-lg overflow-hidden shadow-sm bg-white group-hover:shadow-xl transition-shadow duration-300">
        <Link href={`/off-plan/${project.id}`} className="block">
        ...
        <div className="p-4 flex flex-col flex-grow text-[#111827]">
          {startingPrice > 0 && (
            <p className="text-2xl font-extrabold">
              AED {startingPrice.toLocaleString()}
            </p>
          )}
          <Link href={`/off-plan/${project.id}`} className="block">
          ...
          <div className="mt-2 space-y-2 text-base text-[#4B5563] flex-grow">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">Handover: {project.handover}</span>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t flex justify-between items-center">
            <span className="text-sm font-semibold text-primary">Off-Plan</span>
            <Button
              onClick={(e) => {
                e.stopPropagation();
                window.open(`https://wa.me/?text=I'm interested in the off-plan project: ${project.projectName} (ID: ${project.id})`, '_blank');
              }}
